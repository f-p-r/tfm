...
class UpdateRoleGrantRequest extends FormRequest
{
    /**
     * Determinar si el usuario está autorizado para esta operación.
     * Solo usuarios con rol admin y scope global pueden actualizar role grants.
     */
    ...
    {
        $roleGrant = $this->route('role_grant');
        $scopeType = $this->input('scope_type', $roleGrant->scope_type);
        $scopeId = $this->input('scope_id', $roleGrant->scope_id);

        return [
            'user_id' => ['sometimes', 'required', 'integer', 'exists:users,id'],
            'role_id' => ['sometimes', 'required', 'integer', 'exists:roles,id'],
            'scope_type' => ['sometimes', 'required', 'integer', Rule::in([
            ...
            'scope_id' => [
                ...
                function ($attribute, $value, $fail) use ($scopeType, $scopeId, $roleGrant) {
                    $userId = $this->input('user_id', $roleGrant->user_id);
                    $roleId = $this->input('role_id', $roleGrant->role_id);

                    // Normalizar scope_id (0 -> null para global)
                    $normalizedScopeId = ($scopeType == RoleGrant::SCOPE_GLOBAL && ($value === 0 || $value === '0')) ? null : $value;

                    // Verificar si ya existe este role grant exacto (excluyendo el actual)
    ...
    {
        abort(403, 'No tienes permisos para actualizar role grants. Se requiere rol de administrador.');
    }
}
